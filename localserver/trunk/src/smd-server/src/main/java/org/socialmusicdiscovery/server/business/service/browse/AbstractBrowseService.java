...
public abstract class AbstractBrowseService<T extends SMDIdentity> {
    ...
    protected String buildResultWhereString(String relationName, Collection<String> criteriaList) {
    ...
    protected Query createCountQuery(Class entity, String relationType, String orderBy, Collection<String> criteriaList, Collection<String> sortCriteriaList, String joinString, String whereString) {
        Query query;
    ...
    protected Result<T> findChildren(Class entity, String relationType, String orderBy, Collection<String> criteriaList, Collection<String> sortCriteriaList, Integer firstItem, Integer maxItems, Boolean returnChildCounters) {
        ...
        Long count = null;
        if (maxItems != null) {
            Query countQuery = createCountQuery(entity, relationType, orderBy, criteriaList, sortCriteriaList, joinString, whereString);
        ...
        if (maxItems == null || count > 0L) {
            Query query = createFindQuery(entity, relationType, orderBy, criteriaList, sortCriteriaList, joinString, whereString);
            if (firstItem != null) {
            ...
            for (T item : items) {
                if (returnChildCounters != null && returnChildCounters) {
                    ...
                    for (String relation : relations) {
                        ...
                        if (criteriaList.size() > 0) {
                            countQuery = entityManager.createQuery("SELECT countRelations.referenceType,countRelations.type,count(distinct countRelations.reference) from RecordingEntity as r JOIN r." + relationType + "SearchRelations as searchRelations " + joinString + " JOIN r." + relation + "SearchRelations as countRelations WHERE searchRelations.reference=:item AND " + whereString + " AND countRelations.reference!=:item " + buildExclusionString("countRelations", criteriaList) + " GROUP BY countRelations.referenceType,countRelations.type");
